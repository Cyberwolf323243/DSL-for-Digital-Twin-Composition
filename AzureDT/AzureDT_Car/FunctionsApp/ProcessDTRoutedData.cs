        ...
        public async Task Run([EventGridTrigger] EventGridEvent eventGridEvent)
        {
            ...
            if (client != null)
            {
                if (eventGridEvent != null && eventGridEvent.Data != null)
                {
                    ...
                    double max_rpm = 5000.0D;
                    double max_powerConsumption = 20.0D;
                    ...
                    foreach (var operation in message["data"]["patch"])
                    {
                        ...
                        if (opValue.Equals("replace"))
                        {
                            ...
                            if (propertyPath.Equals("/Utilization"))
                            {
                                int rpm_value = (int)(operation["value"].Value<float>()/100.0F*max_rpm);
                                double powerConsumption_value = operation["value"].Value<float>()/100.0F*max_powerConsumption;
                                await AdtUtilities.UpdateTwinPropertyAsync(client, twinId, "/RPM", rpm_value, _logger);
                                await AdtUtilities.UpdateTwinPropertyAsync(client, twinId, "/PowerConsumption", powerConsumption_value, _logger);
                            }
                        }
                    }
                }
