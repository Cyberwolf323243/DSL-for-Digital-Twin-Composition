    ...
    public class ProcessHubToDTEvents
    {
        ...
        public async Task Run([EventGridTrigger]EventGridEvent eventGridEvent)
        {
            ...
            if (eventGridEvent != null && eventGridEvent.Data != null)
            {
                ...
                // Reading deviceId and utilization for IoT Hub JSON
                JObject deviceMessage = (JObject)JsonConvert.DeserializeObject(eventGridEvent.Data.ToString());
                string deviceId = (string)deviceMessage["systemProperties"]["iothub-connection-device-id"];
                var utilization = deviceMessage["body"]["Utilization"];

                _logger.LogInformation($"Device:{deviceId} Utilization is:{utilization}");

                //Update twin using device utilization
                var updateTwinData = new JsonPatchDocument();
                updateTwinData.AppendReplace("/Utilization", utilization.Value<double>());
